...
# ✅ Zephyr (chat model)
zephyr_endpoint = HuggingFaceEndpoint(
    repo_id="mistralai/Mixtral-8x7B-v0.1",
    task="conversational",
    provider="auto"
)

model1 = ChatHuggingFace(endpoint=zephyr_endpoint)

# ✅ Falcon (text generation model)
model2 = HuggingFaceEndpoint(
...
# -------------------
parallel_chain = RunnableParallel({
    'notes': prompt1 | model1 | parser,   # Zephyr handles notes (chat model)
    'quiz': prompt2 | model2 | parser     # Falcon handles quiz (text gen model)
})
